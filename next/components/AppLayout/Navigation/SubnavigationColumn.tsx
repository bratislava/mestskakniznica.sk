...
const Column = ({ section, latestEvents, classNames }: ColumnProps) => {
  ...
  const containsEvents = section?.sectionLinks?.some(
    (sectionLink) =>
      sectionLink?.sectionLinkPage?.layout === Enum_Page_Layout.Event
  );
  const isLengthy = section?.sectionLinks
    ? section.sectionLinks.length >= 8
    : false;
    ...
    >
      {section.sectionTitle && section.sectionPage !== null && (
        <NavigationMenu.Link
          ...
          tabIndex={-1}
        >
          <Link href={`/${section?.sectionPage?.slug}`}>
        ...
        {section.sectionLinks?.map((sectionLink) => {
          if (sectionLink?.sectionLinkTitle === 'latestEvents') {
                  ...
                  {latestEvents.map((event: any) => (
                    <div key={event.slug}>
                      <div className="w-[380px] pt-5 pb-5 h-23 cursor-pointer">
                        <NavigationMenu.Link
                          className="pt-4 h-10 text-gray-universal-70"
                          tabIndex={-1}
                        >
            ...
            return (
              <NavigationMenu.Link
                className={cx(
                  'pt-4 cursor-pointer text-gray-universal-70 h-auto',
                  {
              ...
              >
                <Link href={`/${sectionLink.sectionLinkPage.slug}`} passHref>
                  <a
                    href={`/${sectionLink.sectionLinkPage.slug}`}

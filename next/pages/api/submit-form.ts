...
if (
  ...
  console.warn(
    'Missing mailgun config properties - sending emails will not work!'
  );
}
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    const { mg_subject, mg_email_to, meta_sent_from, meta_locale, ...rest } =
      body;
    ...
    const dataToSend = {
      ...
      to: mg_email_to,
      subject:
        mg_subject || 'Nová odpoveď na formulár stránky mestskakniznica.sk',
      text,
    };

    await messenger.sendEmail(dataToSend);

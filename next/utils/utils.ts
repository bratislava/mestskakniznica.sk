...
export const dayForDifferentDateTo = (
  dateFrom: Date,
  dateTo: Date,
...
export const convertPageToEventDisplay = (
  ...
  if (page.layout === Enum_Page_Layout.Event) {
    const eventDetails = page.sections?.find(
      (section) => section?.__typename === 'ComponentSectionsEventDetails'
    );
    return {
      ...page.sections?.find(
        (section) => section?.__typename === 'ComponentSectionsEventDetails'
      ...
      eventLocality:
        eventDetails &&
        eventDetails.__typename === 'ComponentSectionsEventDetails'
          ? eventDetails.eventLocality
          : null,
      eventCategory:
        eventDetails &&
        eventDetails.__typename === 'ComponentSectionsEventDetails'
...
export const convertPagesToLocalities = (
  ...
  pages.forEach((page) => {
    const localityDetails = page.sections?.find(
      (section) => section?.__typename === 'ComponentSectionsLocalityDetails'
    );
...
export const convertPageToLocality = (
  ...
  if (page.layout === Enum_Page_Layout.Locality) {
    const { localityOpenFrom, localityOpenTo, isCurrentlyOpen } =
      getMainOpeningHours(localityDetails.localitySections);
...
const getMainOpeningHours = (
  sections: ComponentLocalityPartsLocalityOpeningHours[]
) => {
  ...
  const today = new Date();
  const isCurrentlyOpen = getIsCurrentlyOpen(
    today,
    customArray[today.getDay()]
  );
  ...
  openFrom = `${formatTime(customArray[0].from.hours)}:${formatTime(
    customArray[0].from.minutes
  )}`;
  openTo = `${formatTime(customArray[1].to.hours)}:${formatTime(
    customArray[1].to.minutes
  )}`;
...
export const shouldSkipStaticPaths = () =>
  process.env.NODE_ENV === 'development' || tokenizedSrapiUrlNoAvailable();
